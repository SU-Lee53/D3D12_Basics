...
void D3D12Renderer::RenderSpriteWithTex(shared_ptr<void>& pSpriteHandle, int iPosX, int iPosY, float fScaleX, float fScaleY, const RECT* pRect, float Z, shared_ptr<void>& pTexHandle)
{
	...
	shared_ptr<SpriteObject> pSpriteObj = static_pointer_cast<SpriteObject>(pSpriteHandle);
	...
	pSpriteObj->DrawWithTex(pCommandList, Pos, Scale, pRect, Z, static_pointer_cast<TEXTURE_HANDLE>(pTexHandle).get());
}
