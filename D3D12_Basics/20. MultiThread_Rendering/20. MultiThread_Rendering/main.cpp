    ...
    _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_MY20MULTITHREADRENDERING, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_MY20MULTITHREADRENDERING));

    MSG msg;

    g_pGame = std::make_shared<Game>();
#ifdef _DEBUG
    g_pGame->Initialize(g_hMainWindow, FALSE, FALSE); // 이거 지금 켜면 3프레임나옴;;
#elif _DEBUG
    g_pGame->Initialize(g_hMainWindow, FALSE, FALSE);
#endif

    SetWindowText(g_hMainWindow, L"CommandList Pool");
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDC_MY20MULTITHREADRENDERING));
    wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
    wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_MY20MULTITHREADRENDERING);
