...
BOOL SpriteObject::Initialize(std::shared_ptr<D3D12Renderer> pRenderer, const WCHAR* wchFilename, const RECT* pRect)
{
	...
	if (bResult)
	{
		UINT TexWidth = 1;
		UINT TexHeight = 1;
		m_pTexHandle = std::static_pointer_cast<TEXTURE_HANDLE>(m_pRenderer.lock()->CreateTextureFromFile(wchFilename));

		if (m_pTexHandle.lock())
		{
			D3D12_RESOURCE_DESC desc = m_pTexHandle.lock()->pTexResource->GetDesc();
			TexWidth = desc.Width;
...
{
	BOOL bResult = FALSE;
	ComPtr<ID3D12Device5>& prefeD3DDevice = m_pRenderer.lock()->GetDevice();
	UINT srvDescriptorSize = m_pRenderer.lock()->GetSrvDescriptorSize();
	std::shared_ptr<D3D12ResourceManager>& prefResourceManager = m_pRenderer.lock()->GetResourceManager();
	std::shared_ptr<SingleDescriptorAllocator>& prefSingleDescriptorAllocator = m_pRenderer.lock()->GetSingleDescriptorAllocator();
...
void SpriteObject::CleanUp()
{
	if (m_pTexHandle.lock())
	{
		m_pRenderer.lock()->DeleteTexture(static_pointer_cast<void>(m_pTexHandle.lock()));
	}
