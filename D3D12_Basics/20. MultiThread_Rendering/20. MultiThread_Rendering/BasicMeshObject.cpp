...
{
	BOOL bResult = FALSE;
	ComPtr<ID3D12Device5>& refD3DDevice = m_pRenderer.lock()->GetDevice();
...
BOOL BasicMeshObject::InsertTriGroup(const WORD* pIndexList, DWORD dwTriCount, const WCHAR* wchTexFileName)
{
	...
	pTriGroup->pTexHandle = std::static_pointer_cast<TEXTURE_HANDLE>(m_pRenderer.lock()->CreateTextureFromFile(wchTexFileName));
	m_dwTriGroupCount++;
	bResult = TRUE;
...
void BasicMeshObject::CleanUp()
{
	if (!m_TriGroupList.empty())
	{
		for (DWORD i = 0; i < m_dwTriGroupCount; i++)
		{
			...
			{
				m_pRenderer.lock()->DeleteTexture(static_pointer_cast<void>(m_TriGroupList[i].pTexHandle));
